...
def match_features_train_model_2(df_matches,elo_df):
    df_for_model=prepare_match_features(df_matches,elo_df)
    X=df_for_model[['dif_elo_before','dif_avg_goal_for','away_avg_goals_against',
                    'home_home_loss_rate','home_avg_points_before_match','home_avg_goals_against']]
    y=df_for_model["score.winner"]
    return X,y

def prepare_unplayed_match_features(df,history):
...
def match_features_pred_model_2(df_matches,elo_df,select_matchday=None):
    df_for_model=prepare_unplayed_match_features(df_matches,elo_df)
    if select_matchday is not None:
        df_for_model=df_for_model[df_for_model["matchday"]==select_matchday]
    X=df_for_model[['dif_elo_before','dif_avg_goal_for','away_avg_goals_against',
                    'home_home_loss_rate','home_avg_points_before_match','home_avg_goals_against']]
    y=df_for_model["score.winner"]
    label = df_for_model[['matchday','homeTeam.name','awayTeam.name','homeTeam.id','awayTeam.id']].copy()
    label["game_name"]=label["homeTeam.name"]+" / "+label["awayTeam.name"]
